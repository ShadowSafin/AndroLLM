...
|---|---|---|
| "Sign in failed" with SHA error | SHA fingerprint not registered (check Play App Signing too!) | Add all SHA-256 fingerprints to Firebase Console |
...
You must register **all three** signing keys in Firebase Console:

| Key | Where to get it | When it's used |
|---|---|---|
| Debug keystore | `keytool -list -v -keystore ~/.android/debug.keystore -alias androiddebugkey -storepass android` | Debug builds |
| Upload keystore | Your release keystore (`Mainkeystore`) | Direct APK installs |
| **Play App Signing key** | **Google Play Console → Setup → App Integrity → App signing key certificate** | **AAB installed via Google Play** |

**⚠️ If Play App Signing is enabled (default for new apps), the Play signing key SHA-256 is the one most users will hit. Missing it means Google Sign-In fails for everyone who installs from the Play Store.**
...
3. **SHA fingerprints must match** — debug, release, AND Play App Signing keystores need separate entries in Firebase Console
