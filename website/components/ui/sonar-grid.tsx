...
// Adapted for this project from the upstream 21st.dev source. The only change is
// the canvas color class: upstream uses shadcn's `text-primary`, which is not part
// of this project's Tailwind theme (see the LAYER 1/2/3 token contract at the top of
// app/globals.css). `var(--accent)` is the semantic equivalent — the one brand color —
// so the field still follows the theme's primary color as documented below.
...
export interface SonarGridProps extends React.ComponentProps<"div"> {
  ...
  interactive?: boolean
  /**
   * Local addition, not present upstream. Where the ping listener attaches:
   * "host" is upstream behaviour and only fires over the component's own box.
   *
   * "window" exists because a full-viewport backdrop cannot be a hit target at
   * all. At a negative z-index it paints behind the body box, and `body` is
   * hit-testable, so it always wins the hit test and a host listener never
   * fires. Listening on `window` also intercepts nothing — the event still
   * reaches its real target — whereas a hit-testable overlay would.
   */
  pointerSource?: "host" | "window"
...
export function SonarGrid({
  ...
  const opts = React.useRef({ spacing, dotRadius, baseOpacity, pingEvery, speed, ringWidth, amplitude, interactive, maxRings, seedPing, pingArea, pointerSource })
  opts.current = { spacing, dotRadius, baseOpacity, pingEvery, speed, ringWidth, amplitude, interactive, maxRings, seedPing, pingArea, pointerSource }
  ...
  React.useEffect(() => {
    ...
    const downTarget: EventTarget = opts.current.pointerSource === "window" ? window : host
  ...
  React.useEffect(() => {
    refreshRef.current()
  }, [spacing, dotRadius, baseOpacity, color, pingEvery, speed, ringWidth, amplitude, interactive, maxRings, pingArea, pointerSource])
  ...
  return (
    ...
    >
      <canvas
        ref={canvasRef}
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 -z-10 size-full text-[var(--accent)]"
