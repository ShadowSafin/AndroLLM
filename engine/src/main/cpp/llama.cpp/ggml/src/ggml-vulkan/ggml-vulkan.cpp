...
// Android's minSdk 24 Vulkan linker stub does not export this Vulkan 1.1 core
// entry point, even though modern Android Vulkan loaders expose it. Resolve it
// through the active instance so the backend remains loadable on API 24 and can
// report a clean CPU fallback on devices that do not provide Vulkan 1.1+.
static void ggml_vk_get_physical_device_features2(
    ...
    if (get_features2 == nullptr) {
        throw std::runtime_error("Vulkan 1.1 vkGetPhysicalDeviceFeatures2 is unavailable");
    }
